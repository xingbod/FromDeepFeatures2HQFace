...
for num_repeat in range(3):
    ...
    img_dir = 'create a new folder, put you pictures in it'
    save_dir = 'the path of you save results'
    dirs_name = os.listdir(img_dir)

    for name in dirs_name:
        file_name, extension = os.path.splitext(name)
        dir_path = os.path.join(save_dir, file_name)  # 人名目录
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)

        save_path = dir_path + f"/result{num_repeat}"
        if not os.path.exists(save_path):
            os.mkdir(save_path)

        img = io.imread(os.path.join(img_dir, name))
        img_gt = np.array(img)
        Image.fromarray(img_gt, 'RGB').save(save_path + r'/ground_truth''.png')
